...
def evaluate(sess, p_, y_):
    """测试集上准曲率评估"""
    data_len = len(p_)
    batch_eval = batch_iter(p_, 128)
    total_loss, total_acc = 0, 0
    for batch_p,  batch_y in batch_eval:
        batch_len = len(batch_p)
        loss, acc = sess.run([model.loss, model.acc],
                             feed_dict={model.input_x: batch_p,
                                        model.keep_prob: 1.0})
...
def train(word_to_id, cat_to_id):
    ...
    flag = False
    p_train, y_train = process_file('./snli_1.0/snli_1.0_train.txt', word_to_id, cat_to_id, 20)
    p_val, y_val = process_file('./snli_1.0/snli_1.0_dev.txt', word_to_id, cat_to_id, 20)
    with tf.compat.v1.Session() as sess:
        sess.run(tf.compat.v1.global_variables_initializer())
        for step in range(1000):
            batch_train = batch_iter(p_train, y_train)
            for batch_p, batch_ys in batch_train:
                if total_batch % 100 == 0:
                    loss_train, acc_train = sess.run([model.loss, model.acc],
                                                     feed_dict={model.input_x: batch_p,
                                                                model.input_y: batch_ys,
                                                                model.keep_prob: 0.5})
                    loss_val, acc_val = evaluate(sess, p_val, y_val)
